...
Corpse is not under a roof.

Corpse is reachable by sunlight (GenCelestial.CurCelestialSunGlow(map) > 0).

Corpse is not in a cryptosleep pod or fully buried under terrain.

Corpse may be in a coffin, as long as the coffin itself is exposed to the sun (unroofed, outdoors).
...
Holding the button (if possible) or clicking repeatedly simulates continuous firing — the longer the beam persists, the more SolarCharge is drained.
...
Massive white flash (ThrowLightningGlow)

0.5s delay → PsychicPulseGlobal sound

Fire/smoke wave spreading outward (chain explosions)

Screen shake if possible

Superman collapses (SolarExhaustion)
...
Player crafts Kryptonian Gene Injector.

Pawn uses injector, gaining gene and full SolarCharge.

Powers gizmo appears.

Player uses powers freely; SolarCharge slowly drains.

If SolarCharge = 0 → Superman collapses, enters Solar Exhaustion (12h cooldown).

If killed outdoors → corpse slowly recharges via sunlight, resurrects automatically at 100%.

If buried indoors or roofed → remains inert until moved into sunlight.

Player may unleash Supernova to destroy everything — resetting charge to 0 and entering exhaustion.
